...
export const crearTarea = async (req: Request, res: Response) => {
  ...
  if (isNaN(id_asignado)) {
    ...
    return res.status(400).json({ error: 'El campo id_asignado debe ser un número válido' });
  }
  ...
  try {
    ...
    if (usuarioAsignado.rol !== 'admin') {
      return res.status(400).json({ error: 'El usuario asignado debe tener rol "admin"' });
    }
    ...
    return res.status(500).json({ error: 'Error al crear la tarea', details: error });
  }
};
...
export const actualizarEstado = async (req: Request, res: Response) => {
  ...
  try {
    const tarea = await prisma.tareas.update({  // Cambié de 'tarea' a 'tareas'
...
export const eliminarTarea = async (req: Request, res: Response) => {
  ...
  try {
    await prisma.tareas.delete({  // Cambié de 'tarea' a 'tareas'
      where: { id_tarea: Number(id) }
    });
