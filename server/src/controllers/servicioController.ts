...
export const updateService = async (req: Request, res: Response) => {
  ...
  const { nombre, tipo, descripcion, precio, duracion_estimada, activo } = req.body;
  ...
  try {
    const servicioExistente = await prisma.servicios.findUnique({ where: { id_servicio: id } });
...
export const getServiciosActivos = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: "Error al obtener servicios activos" });
  }
};
...
export const deleteService = async (req: Request, res: Response) => {
  ...
  try {
    const servicioExistente = await prisma.servicios.findUnique({ where: { id_servicio: id_servicio } });
    if (!servicioExistente) {
      return res.status(404).json({ error: 'Servicio no encontrado' });
    }
