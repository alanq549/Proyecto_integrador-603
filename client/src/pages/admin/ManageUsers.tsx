...
const handleRegisterAdmin = async (e: React.FormEvent) => {
  ...
  try {
    const ruta =
      newUser.rol === "admin"
        ? `${API_URL}/registerA`
        : `${API_URL}/empleado`; // Ajusta según tu backend
    ...
    setShowForm(false);
    toast.success(`${newUser.rol === "admin" ? "Administrador" : "Empleado"} registrado exitosamente`);
  } catch (err) {
    setError(err instanceof Error ? err.message : "Error desconocido");
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        >
          {showForm ? "Cancelar" : (newUser.rol === "admin" ? "Nuevo Administrador" : "Nuevo Empleado")}
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-md dark:bg-gray-800 max-w-lg w-full p-6">
            {/* Aquí va tu formulario */}
            <h3 className="text-xl font-semibold mb-4">
              Registrar Nuevo{" "}
              {newUser.rol === "admin" ? "Administrador" : "Empleado"}
            </h3>
      ...
      focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <button
                type="submit"
                className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition disabled:opacity-50 ml-16 "
                disabled={loading.form}
              >
                {loading.form ? "Registrando..." : (newUser.rol === "admin" ? "Registrar Administrador" : "Registrar Empleado")}
              </button>

                      <button
          onClick={() => setShowForm(!showForm)}
          className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition ml-5"
        >
          {showForm ? "Cancelar" : (newUser.rol === "admin" ? "Nuevo Administrador" : "Nuevo Empleado")}
        </button>
            </form>
