...
const OrdenesAdmin = () => {
  ...
  interface OrdenActiva {
    ...
    nombre?: string; // para cliente ocasional
    vehiculo?: {
      placa: string;
      marca: string;
      modelo: string;
...
<form className="space-y-6" onSubmit={handleSubmit}>
  ...
  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
    <div className="flex items-center mb-4">
      <svg className="w-5 h-5 mr-2 text-gray-700 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          <svg className="inline w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                          </svg>
                          {cliente.vehiculos?.length || 0} vehículos registrados
                        </p>
                      </div>
                    </div>
                  </li>
                ...
                ordenesActivas.map((orden) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                      {orden.cliente
                        ? `${orden.cliente.nombre} (Registrado)`
                        : `${orden.nombre} (Ocasional)`}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                      {new Date(orden.fecha_inicio).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {orden.pagos.length > 0 ? (
                        <span
                          className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            orden.pagos[0].estado === "completado"
                              ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300"
                              : "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300"
                          }`}
                        >
                          {orden.pagos[0].estado}
                        </span>
                      ) : (
